...
def eval_run(
    ...
    tuples_per_s: float = round(n_tuples / elapsed_time, 3)
...
def run_horizon(
    ...
    if not (dataset_path / "dirty.csv").exists():
        ...
        if not injected_path.exists():
            print(
                f"No dirty data file found. Skipping benchmarks for dataset {dataset_path.name}."
            )
            return []
        dirty_data_files = [
            str(injected_data).replace(str(dataset_path), "")
    ...
    n_fds: int = sum(1 for line in open(fd_files[0], "r").readlines())
    if fd_files[0].suffix == ".csv":
        n_fds = n_fds - 1
    ...
    for dirty_data_file in dirty_data_files:
        ...
        print(f"\nNow running benchmarks for dataset {dataset_name}.")
        ...
        # TODO: Get repair time directly form horizon.py
        elapsed_time: float = end_time - start_time

        print(f"{dataset_name} completed in {elapsed_time:.2f}s.")

        # Get dataset properties
        error_type, _, error_rate = (
            dataset_path / Path(dirty_data_file)
        ).stem.partition("_r")
        ...
        evaluation: dict = eval_run(
            ...
            dataset_path / Path(dirty_data_file),
            output_path / f"{dataset_name}_cleaned_data.csv",
            elapsed_time,
        )
        # If only no error rate given, calculate via n_dirty / n_tuples
        if (
            dirty_data_properties["error_rate"] == ""
            or dirty_data_properties["error_rate"] is None
        ):
            dirty_data_properties["error_rate"] = (
                round(evaluation["n_dirty"] / evaluation["n_tuples"], 3)
                if evaluation["n_dirty"] is not None
                else None
            )
