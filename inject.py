...
def read_str(source: str | Path, *, has_header: bool = True) -> pl.DataFrame:
    """Read a CSV with every cell as a string (preserve leading zeros, no float reformatting)."""
...
def parse_changes(changes_path: str | Path) -> pl.DataFrame:
    """Parse a headerless BART changes file to columns [__row, attr, dirty, recorded].
...
def apply_changes(clean: pl.DataFrame, changes: pl.DataFrame) -> pl.DataFrame:
    ...
    `changes` must be deduped to one row per cell. Each cell is updated by a
    keyed `update` (never a fan-out join), so duplicate keys could only ever
    overwrite, never multiply rows.
    """
